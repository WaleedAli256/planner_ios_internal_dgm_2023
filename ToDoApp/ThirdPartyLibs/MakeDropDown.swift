...
class MakeDropDown: UIView{
    ...
    var cellReusableIdentifier: String = "DropDownTableViewCell"
    // Table View
    ...
    func setUpDropDown(viewPositionReference: CGRect,  offset: CGFloat){
        ...
        self.frame = CGRect(x: viewPositionReference.midX, y: viewPositionReference.maxY + offset, width: 0, height: 0)
        dropDownTableView = UITableView(frame: CGRect(x: self.frame.minX, y: self.frame.minY, width: 0, height: 0))
        self.width = viewPositionReference.width * 2
        self.offset = offset
    ...
    func showDropDown(height: CGFloat){
        if isDropDownPresent{
            ...
            self.frame = CGRect(x: (self.viewPositionRef?.midX)!, y: (self.viewPositionRef?.maxY)! + self.offset, width: width, height: 0)
    ...
    func reloadDropDown(height: CGFloat){
        self.frame = CGRect(x: (self.viewPositionRef?.midX)!, y: (self.viewPositionRef?.maxY)!
